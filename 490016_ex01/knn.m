...
[k_values index] = sort(D)
ind_viz = index(1:K)

matriz = zeros(size(D), size(D))
specific_diagonals = (size(D)(1) * (ind_viz - 1) + 1)
matriz(specific_diagonals) = 1

versicolour = sum(matriz * Y)
setosa = sum(matriz * ((Y-1)*-1))

if (versicolour > setosa)
  y = 1
else
  y = 0
endif
