...
flag = 0;
  somatorio1 = ((theta(1) + theta(2) * X(1:m, [2])) - y);
  theta(1) = theta(1) - (alpha/m) * (sum(somatorio1));

  somatorio2 = ((theta(1) + theta(2) * X(1:m, [2])) - y) .* X(1:m, [2]);
  theta(2) = theta(2) - (alpha/m) * (sum(somatorio2));
